...
Working with relational data tables doesn't sound like the most exciting topic, but it's one I always could do better in my data science projects. Kirill Müller drew in quite an audience for his overview of the [`{dm}`](https://github.com/cynkra/dm) package at [his rstudio::conf talk in 2022](https://www.rstudio.com/conference/2022/talks/dm-analyze-build-deploy-relational/). `{dm}` is designed to bridge the gap between individual data frames and relational databases, making it a powerful tool for anyone working with large or complex datasets.
...
In this tutorial, we will use the [`{alone}`](https://gradientdescending.com/alone-r-package-datasets-from-the-survival-tv-series/) package, part of week for of #TidyTuesday. Our first step is to convert data in the `{alone}` package into a `dm` object.
